...
class Agent:
    ...
    def get_state(self, game: Game):
        ...
        # i have path info, and I know what paths are dangerous. And I know how the board looks
        # I need to go trough each step and look if all dangerous paths connect to each other
        ...
        for i in range(0, 12):
            if outputDanger[i] != 1:
                ...
                #print("step", i)
                if i < 6:
                    for path in range(0, 12):
                        newEntry = path_info[(path - i * 2) % 12][0]
                        newExit = path_info[(game.board.cur_tile.connects[path] - 2 * i) % 12][0]
                        #print("path", path_info[(path + i * 2) % 12][0],
                        #path_info[(game.board.cur_tile.connects[(path + i * 2) % 12] - 2 * i)%12][0])
                        #print("connect", (path - i * 2) % 12, (game.board.cur_tile.connects[path] - 2 * i)%12)
                        if newEntry == 1 and newExit == 1:
                            if not someConnect:
                                allConnect = True
                            someConnect = True

                        elif newEntry != newExit and (newExit == 1 or newEntry == 1):
                            allConnect = False
                else:
                    for path in range(0, 12):
                        newEntry = path_info[(path - (i - 6) * 2) % 12][0]
                        newExit = path_info[(game.board.swap_tile.connects[path] - 2 * i) % 12][0]
                        if newEntry == 1 and newExit == 1:
                            if not someConnect:
                                allConnect = True
                            someConnect = True
                            ...
                            allConnect = False
                if allConnect and outputScore[i] == 3:
                    outputScore[i] = 6
                elif someConnect and outputScore[i] == 3:
                    outputScore[i] = 4
                elif allConnect:
                    outputScore[i] = 2
                elif someConnect:
